...
export const tokenBudget = pgTable("TokenBudget", {
  ...
  totalBudget: integer("totalBudget").notNull().default(0),
  usedBudget: integer("usedBudget").notNull().default(0),
  createdAt: timestamp("createdAt").notNull().defaultNow(),
  updatedAt: timestamp("updatedAt").notNull().defaultNow(),
})

export type TokenBudget = InferSelectModel<typeof tokenBudget>

export const tokenRequest = pgTable("TokenRequest", {
  id: uuid("id").primaryKey().notNull().defaultRandom(),
  userId: uuid("userId")
    .notNull()
    .references(() => user.id),
